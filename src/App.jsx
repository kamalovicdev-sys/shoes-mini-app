...
const BRAND_LOGOS = [
  ...
  { name: 'New Balance', logo: 'https://logolook.net/new-balance-logo' },
...
function App() {
  ...
  useEffect(() => {
    if (WebApp) {
      WebApp.ready();
      WebApp.expand();
      // Telegram xatolik bermasligi uchun versiya tekshiruvi qo'shdik
  ...
  useEffect(() => {
    ...
    if (isCartOpen) {
      ...
      WebApp.MainButton.show();
      if (WebApp.isVersionAtLeast('6.1')) WebApp.BackButton.hide();
    } else {
  ...
  useEffect(() => {
    ...
    if (WebApp.isVersionAtLeast('6.1')) WebApp.BackButton.onClick(handleBackButtonClick);

    return () => {
      WebApp.MainButton.offClick(handleMainButtonClick);
      if (WebApp.isVersionAtLeast('6.1')) WebApp.BackButton.offClick(handleBackButtonClick);
    };
    ...
    <div className="app-container">
      ...
      {!isCartOpen && !isCheckoutOpen && (
        <>
          ...
          ) : (bestSellers.length > 0 || newArrivals.length > 0) ? ( /* <--- MANA SHU QATOR XATONI TO'G'IRLADI */
            <>
                      ...
                      return (
                        <div key={shoe.id} className={`shoe-card ${isInCart ? 'in-cart' : ''}`}>
                          ...
                          <div className="card-info" onClick={() => setDetailsModal(shoe)}>
                            ...
                            <div className="price-section"><span className={`price ${shoe.isDeal ? 'deal-price' : ''}`}>{shoe.price}</span></div>
                          </div>
